...
export function Stats({ data, error }:any) {
  const { classes } = useStyles();
  const [date, setDate] = useState(new Date(2021, 9, 24));
  const totalContributions = data ? data.user.contributionsCollection.totalCommitContributions : null
  const totalPullRequestContributions = data ? data.user.contributionsCollection.totalPullRequestContributions : null

  const stats = data.map((data.user.contributionsCollection.contributionCalendar.weeks) => (
